...
def printProgressBar(iteration, total, start_time, length=50):
    ...
    os.system('cls')
    print(f"\r|{bar}| {percent:.2f}% complete, ETA: {remaining_time:.2f} seconds")

def systemFromBits(bits, start_time):
    global valid, invalid
    if not (valid + invalid) % 100:
        printProgressBar(valid + invalid, 62336617, start_time)
        print(valid + invalid)
    currSystem = []
    for line in bits:
        currBlock = []
            ...
            if line.startswith("$"):
                ...
                for i in range(soln_size):
                    ...
                    for item in b:
                        chunk.append(item)
                        if len(chunk) == 70:
                            systemFromBits(chunk, start_time)
